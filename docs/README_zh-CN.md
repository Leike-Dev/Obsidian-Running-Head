...
<div align="center">
  ...
  ![Version](https://img.shields.io/badge/version-2.1.1-lightgreen.svg)
...
* **🚫 隐藏第一个 H1**：如果您使用将文件名同步到 H1 的插件，可以防止屏幕上出现重复的标题。
...
4. **配置标签导航**：在 **标签导航** 下，定义一个全局属性名称（例如 `menu`），创建一个标签组（例如 `project`）并添加您的标签。然后，在笔记的 frontmatter 中，使用 `menu: project`。
